    ...
    State<K, T>
{
        ...
        Q: Ord + Hash + Eq + Debug + Sync,
    {
        ...
        metrics
            .sum_store_size
            .add(-(TryInto::<i64>::try_into(item_size).unwrap()), labels);

        if replaced {
            metrics.replaced_items.add(1, labels);
            metrics.replaced_bytes.add(item_size, labels);
        } else {
            metrics.evicted_items.add(1, labels);
            metrics.evicted_bytes.add(item_size, labels);
            metrics.item_size_bytes.record(item_size, labels);
        }
    ...
    async fn put(
        ...
        if let Some(old_item) = self.lru.put(key.clone(), eviction_item) {
            self.remove(&key, &old_item, true, labels).await;
...
> {
    ...
    max_count: u64,
    /// Pre-allocated labels for metrics.
...
where
    ...
    I: InstantWrapper,
{
    pub fn new(config: &EvictionPolicy, anchor_time: I, labels: &[KeyValue]) -> Self {
        let mut metric_labels = vec![
            ...
            //                    using the DAG structure implied by the config.
            //
            //                    In the meantime, the allocation here is not
            //                    the end of the world as it only happens once
            //                    during store construction.
            KeyValue::new("instance_name", "default"),
        ];
        metric_labels.extend_from_slice(labels);
    ...
    async fn evict_items(&self, state: &mut State<K, T>) {
        ...
        while self.should_evict(state.lru.len(), peek_entry, state.sum_store_size, max_bytes) {
            ...
            debug!(?key, "Evicting",);
            state
                .remove(&key, &eviction_item, false, &self.metric_labels)
                .await;
        ...
        Q: Ord + Hash + Eq + Debug + Sync,
    {
        ...
        for (key, result) in keys.into_iter().zip(results.iter_mut()) {
            metrics.cache_requests.add(1, &self.metric_labels);
                    ...
                    if self.should_evict(lru_len, entry, 0, u64::MAX) {
                        *result = None;
                        if let Some((key, eviction_item)) = state.lru.pop_entry(key.borrow()) {
                            ...
                            state
                                .remove(key.borrow(), &eviction_item, false, &self.metric_labels)
                                .await;
                        }
                    } else {
                        if !peek {
                        ...
                        *result = Some(data_len);
                        ...
                        metrics.read_bytes.add(data_len, &self.metric_labels);
                        metrics
                            .item_size_bytes
                            .record(data_len, &self.metric_labels);
                    }
                }
                None => *result = None,
            }
        ...
        <It as IntoIterator>::IntoIter: Send,
    {
        ...
        for (key, data) in inserts {
            ...
            if let Some(old_item) = state.put(key, eviction_item, &self.metric_labels).await {
            ...
            metrics
                .sum_store_size
                .add(new_item_size.try_into().unwrap(), &self.metric_labels);
            metrics
                .inserted_bytes
                .add(new_item_size, &self.metric_labels);
            metrics.inserted_items.add(1, &self.metric_labels);
            metrics
                .item_size_bytes
                .record(new_item_size, &self.metric_labels);
        ...
        Q: Ord + Hash + Eq + Debug + Sync,
    {
        ...
        if let Some(entry) = state.lru.pop(key.borrow()) {
            state.remove(key, &entry, false, &self.metric_labels).await;
            return true;
        }
        false
    }
