	...
	cilium, err := helmv3.NewRelease(ctx, name, &helmv3.ReleaseArgs{
		...
		Values: pulumi.Map{
			...
			"hubble": pulumi.Map{
				...
				"metrics": pulumi.Map{
					//    "enabled": pulumi.String("{dns,drop,tcp,flow,port-distribution,icmp,httpV2:exemplars=true;labelsContext=source_ip\\,source_namespace\\,source_workload\\,destination_ip\\,destination_namespace\\,destination_workload\\,traffic_direction}"),
